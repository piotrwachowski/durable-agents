...
### Added
- Per-tool execution options (`@tool(timeout_seconds=..., max_retries=...)`) are
  now applied when a tool is dispatched. The values are resolved from the
  registry and carried on `ItemResult` so they are applied deterministically on
  workflow replay.
